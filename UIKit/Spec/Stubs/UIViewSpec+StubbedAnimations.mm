...
describe(@"UIView+StubbedAnimation", ^{
    ...
    describe(@"with animations paused", ^{
        ...
        it(@"should records the animation", ^{
            [[[UIView animations] lastObject] duration] should equal(0.666);
        });

        describe(@"running animations", ^{
            beforeEach(^{
                [[[UIView animations] lastObject] animate];
            });
        ...
        describe(@"completing", ^{
            beforeEach(^{
                [[[UIView animations] lastObject] animate];
                [[[UIView animations] lastObject] complete];
            });

            it(@"should run the completion block", ^{
                completionBlockCalled should be_truthy;
            });
        });
        ...
        describe(@"cancelling", ^{
            beforeEach(^{
                [[[UIView animations] lastObject] cancel];
            });
